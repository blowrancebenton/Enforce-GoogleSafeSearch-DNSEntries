...
$ForceSafeSearchIP = [System.Net.Dns]::GetHostAddresses("forcesafesearch.google.com").IPAddressToString
...
ForEach($GoogleDomain in $GoogleDomains){
    ...
    if (!($ZoneFound)) {
        Try {
            ...
            Write-Host "An error occurred creating the DNS zone for www$GoogleDomain"
            $ErrorOccurred = $true
            $ZoneFound = $false
        }
    }
    ...
    If ($ZoneFound) {
        If ($CNameMethodCompatible){
            ...
            Try {
                if ((Get-DnsServerResourceRecord -ZoneName "www$GoogleDomain" -name "@" -RRType CNAME -ComputerName $AD_DNS_Server | select HostName) -ne $null) { $CNameFound = $true }
            } Catch {
                $CNameFound = $false
            }
            ...
            if (!($CNameFound)) {
                Try {
                    Write-Verbose "Attempting to create CName record for www$GoogleDomain"
                    #Try to create the CNAME record - Compatible with Windows Server 2003 Domain Functional Level and below
                    Add-DnsServerResourceRecordCName -ZoneName "www$GoogleDomain" -Name "@" -HostNameAlias "forcesafesearch.google.com" -ComputerName $AD_DNS_Server
                    $ErrorOccurred = $false
                } Catch {
                    Write-Verbose "CName record creation failed for www$GoogleDomain"
        ...
        If (!($CNameMethodCompatible)){
                Try {
                    if ((Get-DnsServerResourceRecord -ZoneName "www$GoogleDomain" -name "@" -RRType A -ComputerName $AD_DNS_Server | select HostName) -ne $null) { $ARecordFound = $true }
                } Catch {
                  $ARecordFound = $false
                }
                ...
                If (!($ARecordFound)) {
                    Try {
                        ...
                        Write-Verbose "Attempting to create A record for www$GoogleDomain with $ForceSafeSearchIP"
                        Add-DnsServerResourceRecordA -ZoneName "www$GoogleDomain" -Name "@" -IPv4Address "$ForceSafeSearchIP" -ComputerName $AD_DNS_Server
                        $ErrorOccurred = $false
                    } Catch {
                        Write-Verbose "Failed to create A record for www$GoogleDomain"
                        Write-Host "An error occurred creating DNS ""A"" record for www$GoogleDomain"
                        $ErrorOccurred = $true
                    }
                }
                if ($ARecordFound) {
                    Try {
                        Write-Verbose "Attempting to remove A record for www$GoogleDomain to update IP"
                        #A record creation failed. Does it already exist?  Remove it and add it back with the latest IP address.
                        Remove-DnsServerResourceRecord -ZoneName "www$GoogleDomain" -Name "@" -RRType "A" -ComputerName $AD_DNS_Server -Force
                        $ErrorOccurred = $false
                    } Catch {
                        Write-Verbose "Failed to remove existing A record for www$GoogleDomain"
                        Write-Host "An error occurred removing (to update) DNS ""A"" record for www$GoogleDomain"
                        $ErrorOccurred = $true
                    }
                    Start-Sleep -m 300 #Pause a few hundred milliseconds and add then add the record back
                    Try {
                        Write-Verbose "Attempting to create new A record for www$GoogleDomain with $ForceSafeSearchIP"
                        Add-DnsServerResourceRecordA -ZoneName "www$GoogleDomain" -Name "@" -IPv4Address "$ForceSafeSearchIP" -ComputerName $AD_DNS_Server
                        $ErrorOccurred = $false
                    } Catch {
                        Write-Verbose "Failed to create/update A record for www$GoogleDomain"
                        Write-Host "An error occurred updating DNS ""A"" record for www$GoogleDomain"
